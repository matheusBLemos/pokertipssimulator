...
func (r *SQLiteRoomRepository) Create(ctx context.Context, room *entity.Room) error {
	...
	_, err = r.db.ExecContext(ctx,
		"INSERT INTO rooms (id, code, mode, data, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
		room.ID, room.Code, string(room.Mode), string(data), now, now,
	)
	return err
}
...
func (r *SQLiteRoomRepository) Update(ctx context.Context, room *entity.Room) error {
	...
	_, err = r.db.ExecContext(ctx,
		"UPDATE rooms SET code = ?, mode = ?, data = ?, updated_at = ? WHERE id = ?",
		room.Code, string(room.Mode), string(data), room.UpdatedAt, room.ID,
	)
	return err
}
