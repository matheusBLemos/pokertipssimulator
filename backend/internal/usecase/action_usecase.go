...
func NewActionUseCase(repo repository.RoomRepository) *ActionUseCase {
...
func (uc *ActionUseCase) validateAction(round *entity.Round, player *entity.Player, ps *entity.PlayerState, actionType entity.ActionType, amount int) error {
	...
	case entity.ActionBet:
		if round.CurrentBet > 0 {
			return entity.ErrInvalidAction // should be raise
		}
	...
	case entity.ActionRaise:
		if round.CurrentBet == 0 {
			return entity.ErrInvalidAction // should be bet
		}
...
func (uc *ActionUseCase) advanceTurn(room *entity.Room, round *entity.Round) {
	...
	if nonFolded <= 1 {
		...
		round.CurrentTurn = ""
		if lastStanding != "" {
			// Auto-win: collect bets and award pot
			totalPot := 0
			for _, ps := range round.PlayerStates {
	...
	// Check if betting round is complete (all active players have acted and bets are equalized)
	allActed := true
	for _, ps := range round.PlayerStates {
