...
func NewGameUseCase(repo repository.RoomRepository) *GameUseCase {
...
func (uc *GameUseCase) StartRound(ctx context.Context, roomID, playerID string) (*entity.Room, error) {
	...
	// Set first to act (player after BB, or after dealer in heads-up)
...
func (uc *GameUseCase) AdvanceStreet(ctx context.Context, roomID, playerID string) (*entity.Room, error) {
	...
	// Reset player states for new street
...
func (uc *GameUseCase) SettleRound(ctx context.Context, roomID, playerID string, req dto.SettleRequest) (*entity.Room, error) {
	...
	for _, w := range req.Winners {
		...
		for i, pid := range w.PlayerIDs {
			...
			if player != nil {
				winAmount := share
				if i == 0 {
					winAmount += remainder // odd chips to first winner
				}
	...
	// Eliminate players with zero stack in tournament mode
...
func (uc *GameUseCase) setFirstToAct(room *entity.Room, round *entity.Round, activePlayers []entity.Player) {
	...
	var firstIdx int
	if len(activePlayers) == 2 {
		firstIdx = dealerIdx // In heads-up preflop, dealer (SB) acts first
	} else {
		firstIdx = (dealerIdx + 3) % len(activePlayers) // UTG
	}
...
func (uc *GameUseCase) calculatePots(round *entity.Round) []entity.Pot {
	...
	for i, b := range bets {
		...
		// Also add non-folded players who bet exactly this level
		for _, pb := range bets {
			if !pb.folded && pb.totalBet == level {
				found := false
